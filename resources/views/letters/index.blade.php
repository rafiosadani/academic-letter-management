    ...
    <div class="mt-4 grid grid-cols-12 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
        <div class="col-span-12">
            ...
            <div class="card mt-3 p-4">
                <form method="GET" action="{{ route('letters.index') }}" class="grid grid-cols-1 gap-2 sm:gap-4 sm:grid-cols-3">
                    ...
                    <div class="flex self-end items-center justify-between sm:justify-start space-x-2">
                        <button type="submit"
                                class="btn w-full sm:w-fit h-8 bg-primary text-xs-plus text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90">
                            <i class="fa-solid fa-filter mr-2"></i>
                            Filter
                        </button>
                        <a href="{{ route('letters.index') }}"
                           class="btn h-8 bg-slate-150 font-medium text-slate-800 hover:bg-slate-200">
            ...
            <div class="card mt-3 p-4">
                            ...
                            <tr class="text-xs border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-tiny text-slate-400 dark:text-navy-300">
                                                {{ $letter->semester->name }} - {{ $letter->academicYear->name }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="flex flex-col">
                                        <span class="text-slate-700 dark:text-navy-100">{{ $letter->created_at->format('d M Y') }}</span>
                                        <span class="text-tiny text-slate-400 dark:text-navy-300">{{ $letter->created_at->format('H:i') }} WIB</span>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5 text-center">
                                    <span class="badge rounded-full bg-{{ $letter->status_badge }}/10 text-{{ $letter->status_badge }} text-tiny">
                                        {{ $letter->status_label }}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5 text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        @can('delete', $letter)
                                            <form action="{{ route('letters.destroy', $letter) }}" method="POST" class="inline-block"
                                                  onsubmit="return confirm('Yakin ingin menghapus pengajuan surat ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                ...
                {{-- Pagination --}}
                @if($letters->hasPages())
                    <div class="flex justify-center border-t border-slate-200 p-4 dark:border-navy-500">
                        {{ $letters->links() }}
                    </div>
                @endif
            </div>
