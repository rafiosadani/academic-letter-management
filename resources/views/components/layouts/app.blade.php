            ...
            @if($hasPanel && !empty($currentPanelMenus))
                <x-slot name="panel">
                    <x-sidebar.panel :title="$panelTitle">
                        <ul class="mt-1 space-y-1.5 font-inter font-medium">
                            @foreach($currentPanelMenus as $submenu)
                                ...
                                <li>
                                    <a href="{{ $submenu['route'] }}"
                                       ...
                                       class="nav-link flex py-2 text-xs+ tracking-wide outline-hidden transition-colors duration-300 ease-in-out {{ $isActive ? 'font-medium text-primary dark:text-accent-light' : 'text-slate-600 hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }}">
                                        {{ $submenu['text'] }}
                                    </a>
                                </li>
                            @endforeach
