...
<div class="document-list-wrapper">
    ...
    @if(count($documents) > 0)
                ...
                <div class="group flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3 transition-colors hover:bg-slate-100 dark:border-navy-500 dark:bg-navy-600 dark:hover:bg-navy-500">
                    ...
                    <div class="flex items-center space-x-2 flex-shrink-0">
                        ...
                        @if($canDelete)
                            <button
                                    type="button"
                                    data-modal-trigger="delete-document-modal-{{ $document->id }}"
        ...
        {{-- Delete Modals --}}
        @foreach($documents as $document)
            <form id="delete-document-form-{{ $document->id }}" method="POST" action="{{ route('documents.destroy', $document) }}" class="hidden">
                @csrf
                @method('DELETE')
            </form>
            ...
            <x-modal.confirm
                    ...
                    title="Hapus Dokumen"
                    message="Yakin ingin menghapus dokumen '{{ $document->file_name }}'?"
                    confirm-type="danger"
                    confirm-text="Hapus"
