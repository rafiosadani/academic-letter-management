...
class CreateRoleRequest extends FormRequest
{
    ...
    {
        // Cek apakah ada error pada field 'permissions'
        if ($validator->errors()->has('permissions')) {
            // Flash notifikasi kustom (Toastify) ke sesi
            session()->flash('notification_data', [
                'type' => 'error',
                // Mengambil pesan error pertama untuk permissions
                'text' => $validator->errors()->first('permissions'),
                'position' => 'center-top',
                'duration' => 4000,
            ]);
        }
        ...
        // Ini yang akan menyebabkan redirect back() dan mengisi $errors
