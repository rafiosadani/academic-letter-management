...
class UpdateFacultyOfficialRequest extends FormRequest
{
    /**
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            $facultyOfficial = $this->route('faculty_official');
            ...
            // 2. Validate study_program_id should match user's home program (optional)
            if ($this->study_program_id && $this->user_id) {
                ...
                if ($user && $user->profile && $user->profile->study_program_id) {
                    if ($user->profile->study_program_id != $this->study_program_id) {
                        // Optional warning - can be enabled if needed
                    }
                }
            }

            // 3. Check for overlapping periods (exclude current record)
            ...
            // 4. Check if user already has active assignment for this position (exclude current)
            if ($this->user_id && $this->position && !$this->end_date) {
                $hasActive = FacultyOfficial::where('user_id', $this->user_id)
                    ->where('position', $this->position)
                    ->where('id', '!=', $facultyOfficial->id)
                    ->active()
                    ->exists();
