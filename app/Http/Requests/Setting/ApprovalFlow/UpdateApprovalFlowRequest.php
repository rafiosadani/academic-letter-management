...
class UpdateApprovalFlowRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if (!$letterType || !$approvalFlow) {
                return;
            }

            // 1. Check for duplicate step (excluding current record and soft deleted)
            ...
            // 2. Check only 1 final step per letter type (excluding current if previously final)
            ...
            // 3. For Word format (SKAK), certain fields are not needed
            if ($letterType->isExternal()) {
                ...
                // on_reject set default if not provided
                if (!$this->on_reject) {
