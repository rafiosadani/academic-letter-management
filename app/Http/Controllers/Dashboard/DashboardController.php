...
class DashboardController extends Controller
{
    ...
    public function index(): View
    {
        ...
        if ($user->hasRole('Kepala Subbagian Akademik')) {
            return $this->kasubbagDashboard();
        }

        if ($user->hasRole('Wakil Dekan Bidang Akademik')) {
            return $this->wakilDekanDashboard();
        }
    ...
    protected function studentDashboard(): View
    {
        ...
        return view('dashboard.student', compact('user', 'student', 'stats', 'recentLetters', 'inProgressLetters'));
    }
    ...
    protected function staffDashboard(): View
    {
        ...
        $stats = $this->dashboardService->getStaffStats($user->id);
        ...
        return view('dashboard.staff', compact('user', 'stats', 'pendingApprovals', 'recentActivity'));
    }

    /**
     * Kasubbag Dashboard
     */
    protected function kasubbagDashboard(): View
    {
        $user = auth()->user();

        $stats = $this->dashboardService->getKasubbagStats($user->id);
        $pendingApprovals = $this->dashboardService->getPendingApprovalsForKasubbag($user->id);
        $flowMetrics = $this->dashboardService->getApprovalFlowMetrics();
        $bottleneckAnalysis = $this->dashboardService->getBottleneckAnalysis();

        return view('dashboard.kasubbag', compact('user', 'stats', 'pendingApprovals', 'flowMetrics', 'bottleneckAnalysis'));
    }

    /**
     * Wakil Dekan Dashboard
     */
    protected function wakilDekanDashboard(): View
    {
        $user = auth()->user();

        $stats = $this->dashboardService->getWakilDekanStats();
        $pendingApprovals = $this->dashboardService->getPendingApprovalsForWakilDekan($user->id);
        $prodiStats = $this->dashboardService->getStatsByProdi();
        $trendData = $this->dashboardService->getTrendData();
        $insights = $this->dashboardService->getInsights();

        return view('dashboard.wakil-dekan', compact('user', 'stats', 'pendingApprovals', 'prodiStats', 'trendData', 'insights'));
    }
    ...
    protected function adminDashboard(): View
    {
        ...
        $activityLog = $this->dashboardService->getRecentActivityLog(20);
        $usageData = $this->dashboardService->getSystemUsageData();

        return view('dashboard.admin', compact('user', 'stats', 'systemHealth', 'userDistribution', 'activityLog', 'usageData'));
    }
    ...
    public function getData(Request $request): JsonResponse
    {
        ...
        if ($user->hasRole('Mahasiswa')) {
            $data = $this->dashboardService->getStudentStats($user->id);
        ...
        if ($user->hasRole('Staf Akademik')) {
            $data = $this->dashboardService->getStaffStats($user->id);
            return response()->json([
        ...
        if ($user->hasRole('Kepala Subbagian Akademik')) {
            $data = $this->dashboardService->getKasubbagStats($user->id);
            return response()->json([
                'pending' => $data['summary']['pending'],
                'approved_today' => $data['summary']['approved_today'],
                'total_month' => $data['summary']['total_month'],
        ...
        if ($user->hasRole('Wakil Dekan Bidang Akademik')) {
            $data = $this->dashboardService->getWakilDekanStats();
            return response()->json([
